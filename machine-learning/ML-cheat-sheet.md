...
### Miscalleneous concepts related to ML
...
- Two types of loss function:
	1) For Regression:
		- Simplest type since you can compare two numerical values by substrcting them f.eks.
...
- **Precision**: High precision --> Minimizes False Positives
	- I.e., doesn't predict someone to be sick (Positive) when they are heathly (Negative)
...
- In conclusion, you should use:
	...
	- Other metrics like Area Under the Curve (**AUC**) or Receiving Operating Characterics (**ROC**) are also very important!
...
- **IMPORTANT QUESTION**: How does 'training' actually work for ML/DL algorithms?
	...
	4) Calculate the **gradient** of the error/loss function with respect to each weight.
		- Without jargon, what does that mean?
			- Simply, the **gradient of the loss function** (or any function in gneeral) indicates the **direction** and **rate** at which the **loss function increases most rapidly**. 
...
- **Types of Gradient Descent algorithms**:
	1) Batch Gradient Descent;
		...
		- Cons:
			...
			- Requires huge amount of resources to fit all the dataset.
	2) Stochastic Gradient Descent:
		- You randomly take one observation from the dataset to update compute the gradient and then update your weight.
			...
			- Cons:
				- Can favor few observations resulting in a temporarily increase in the loss function before reaching the minima (shark-teeth-like function).
	...
	3) Mini-batch Gradient Descent:
		- Combination of both previous types:
			- Take a predefined number of samples/observations randomly selected from the dataset (**batch-size**, which is an user-defined HP) to update the parameters.
		- Kinda the best of both worlds:
			- Randomness helps to explore more the parameter space of the loss function (i.e., avoids local minima)
